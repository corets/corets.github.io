...
Create a manually triggered, async operation.
...
const Example = () => {
  ...
  const handleLike = async () => {
    if ( ! likeAction.isRunning) {
      const result = await likeAction.run(likesCount + 1)

      if ( ! likeAction.isErrored) {
  ...
  return (
    <div>
      { likeAction.isRunning && "Loading..."}
      { likeAction.isErrored && `There was an error: ${likeAction?.error.toString()}` }
