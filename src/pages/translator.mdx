...
- [Static translations access](#createTranslatorAccessor) - **no need for string based keys**
...
```ts
import { createTranslator, defaultTranslatorFormatter } from "@corets/translator"
...
Create a statically typed facade powered by the [@corets/accessor](https://corets.github.io/accessor) package. This allows you to access translations in a statically typed manner, no need to use string based translation keys anymore - missing translations will lead to a compilation error!
...
// pass in a translations object that should be used as source of truth,
// usually this is the default language
...
**Note:** Check out the accessor [docs](https://corets.github.io/accessor) to read more about it's API and features.
...
translator.config({
  ...
  fallbackLanguage: "de",
  // should translations be interpolated? (default: true)
  interpolate: true,
