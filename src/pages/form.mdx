...
This library is not designed for any specific framework or platform. You should be able to use it everywhere where you can run JavaScript. Considering those limitations, it does not offer any logic on how to connect to the input fields out of the box. Package [@corets/use-form-binder](/use-form-binder) provides bindings for vanilla HTML elements, but this won't be enough, most likely.
...
When writing unit tests, make sure that you set the [`debounceChanges`](#formconfig) config property to `0`. This will disable all the throttling of the state changes, and you will be able to write your tests in a synchronous manner.
...
const form = createForm()
  .config({
    // runs sanitizers from coming from schema?
    // this only applies to a complete validation, and not to the reactive one,
    // where validation is run when a field has been changed
    // (default: true)
    sanitize: true,
    // should form validate before submitting? (default: true)
    validateOnSubmit: true,
    // should run validation whenever a field changes?
    // validates only the changed field, not the whole form (default: true)
    validateOnChange: true,
    // should validation be run only against the fields that have actually been changed? (default: false)
    validateChangedFieldsOnly: false,
    // delay notification of listeners (default: 10)
    debounceChanges: 10,
    // same as Form.validator() (default: undefined)
    validator: async (form) => null,
    // same as Form.schema()  (default: undefined)
    schema: object(),
    // same as Form.handler()  (default: undefined)
    handler: async (form) => null,
  })
```
...
You can validate only those fields that have actually been changed (this can also be configured during the form creation):

```ts
const errors = await form.validate({ validateChangedFieldsOnly: true })
```

Disable sanitization that might come from the attached schema object:

```ts
const errors = await form.validate({ sanitize: false })
```

If you decide to validate changed fields only, you can force validation to discard any previous errors:

```ts
const errors = await form.validate({ validateChangedFieldsOnly: true, keepPreviousErrors: false})
```

Just return validation errors, without actually persisting them on the form object:

```ts
const errors = await form.validate({ persistErrors: false })
```
...
You can submit a form without validation like this:

```ts
const result = form.submit({ validate: false })
```

Every option that you can pass to the [`Form.validate()`](#formvalidate) method, you can also pass to the [`Form.submit()`](#formsubmit):

```ts
const result = form.submit({ validateChangedFieldsOnly: true })
```
